...
pxmatrix_gpio = pxmatrix_ns.class_('PxmatrixDisplay', display.DisplayBuffer)
...
CONFIG_SCHEMA = cv.All(
    display.FULL_DISPLAY_SCHEMA.extend({
        ...
        cv.Optional(CONF_PIN_LATCH, default="GPIO14"): pins.gpio_output_pin_schema,
        cv.Optional(CONF_PIN_A, default="GPIO23"): pins.gpio_output_pin_schema,
        cv.Optional(CONF_PIN_B, default="GPIO15"): pins.gpio_output_pin_schema,
        cv.Optional(CONF_PIN_C, default="GPIO5"): pins.gpio_output_pin_schema,
        ...
        cv.Optional(CONF_PIN_OE, default="GPIO26"): pins.gpio_output_pin_schema,
...
def to_code(config):
    ...
    if CONF_LAMBDA in config:
        lambda_ = yield cg.process_lambda(config[CONF_LAMBDA], [(display.DisplayRef, 'it')],
                                          return_type=cg.void)
    ...
    cg.add_library("Wire", "2.0")
    cg.add_library("Spi", "2.0")
    cg.add_library("Ticker", "2.0")
    cg.add_library("Adafruit BusIO", "1.16.1")
